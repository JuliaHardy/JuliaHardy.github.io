...
export class EventFormComponent implements OnDestroy{
  @Output() newDataToDisplay = new EventEmitter<{name: string, endDate: moment.Moment}>();
  ...
  constructor(private readonly store: Store) {
    this.store.select(fromRoot.getCurrentEvent).pipe(takeUntil(this.destroy$)).subscribe(
      data => {
        if(data.currentEvent) {
          this.eventName = data.currentEvent.name as string;
          this.eventDate = moment(data.currentEvent.endDate);
          setTimeout(() => this.newDataToDisplay.emit({name: this.eventName, endDate: this.eventDate!}), 0);
        }
      }
    )
  }
  ...
  public onSubmit() {
    if(this.eventDate && this.eventName) {
      this.newDataToDisplay.emit({name: this.eventName, endDate: this.eventDate});
      this.store.dispatch(eventActions.createEvent({name: this.eventName, endDate: this.eventDate.format()}))
    }
  }
